...
-- MAGIC %md ## Bloom filter index demo
-- MAGIC
-- MAGIC A Bloom filter can tell you if a key is in a set and with a false positive percentage (fpp), if an item _might be_ in a set.
-- MAGIC When used in an index, a Bloom filter can help speed 'needle in a haystack' type queries on fields not sped up by other techniques.
-- MAGIC
-- MAGIC This notebook:
-- MAGIC
-- MAGIC * Sets up a table, Bloom filter index on the table, and fills the table with hashes.
-- MAGIC * Performs a 'needle in a haystack' query:
-- MAGIC   * on a non-indexed column
-- MAGIC   * on an indexed column 
-- MAGIC   * on an indexed column that expects to return zero results (what if there are no needles in the haystack)
-- MAGIC * Cleans up the sample data set

-- COMMAND ----------

-- MAGIC %md ## Create Table, Index, and Load Data
...
-- MAGIC %fs ls dbfs:/tmp/bloom_test/_delta_index/
...
-- DBTITLE 1,Stop the notebook from automatically cleaning up results
